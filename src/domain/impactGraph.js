...
export function nodesInSheet(nodes, sheetId) {
    return nodes.filter((n) => n.sheetId === sheetId);
}
export function edgesTouchingNodes(edges, nodeIds) {
...
export function fullPathLabel(nodes, nodeId) {
...
export function buildPathMap(nodes) {
    const byId = nodeById(nodes);
    const cache = new Map();
...
export function sceneNodes(nodes, focusNodeId, activeSheetId) {
    return nodes
        .filter((n) => n.parentId === focusNodeId &&
...
export function sceneOwnerOf(nodes, focusNodeId, nodeId) {
    const byId = nodeById(nodes);
    let current = byId.get(nodeId);
    const seen = new Set();
    while (current && !seen.has(current.id)) {
...
export function buildAggregatedEdges(nodes, edges, focusNodeId) {
...
function reachableIds(edges, startId, direction) {
...
export function computeImpactHighlight(nodes, edges, aggregatedEdges, focusNodeId, selectedNodeId, direction) {
    let visited;
    if (direction === "both") {
    ...
    const highlightedOwnerIds = new Set();
    ...
    for (const agg of aggregatedEdges) {
        const isRelevant = agg.edges.some((edge) => visited.has(edge.sourceId) && visited.has(edge.targetId));
        if (isRelevant) {
...
export function computeImpactRangeGraph(nodes, edges, selectedNodeId, direction) {
    let visited;
    if (direction === "both") {
        const down = reachableIds(edges, selectedNodeId, "downstream");
        const up = reachableIds(edges, selectedNodeId, "upstream");
...
export function parseGraphImport(text) {
    let data;
    try {
    ...
    let unsortedSheet = null;
    function ensureUnsortedSheet() {
        if (!unsortedSheet) {
    ...
    const placeholderSheetIds = new Set();
    let unsortedNodeCount = 0;
    function resolveRootSheetId(root) {
        ...
        if (root.sheetId) {
            if (!placeholderSheetIds.has(root.sheetId)) {
                ...
                warnings.push(`シートID "${root.sheetId}" が見つからないため仮のシートを作成しました。`);
            }
            return root.sheetId;
        }
        unsortedNodeCount += 1;
    ...
    if (unsortedNodeCount > 0) {
        warnings.push(`${unsortedNodeCount}件のノードにシート情報がなかったため「${UNSORTED_SHEET_NAME}」シートに割り当てました。`);
    }
    if (sheets.length === 0) {
        const now = new Date().toISOString();
        sheets.push({
    ...
    for (const raw of obj.edges) {
        ...
        if (!nodeByIdMap.has(e.sourceId) || !nodeByIdMap.has(e.targetId)) {
            warnings.push(`関係「${e.name}」は参照先ノードが見つからないため読み込みをスキップしました` +
...
export function buildExportRows(allNodes, nodes, edges, sheets) {
    ...
    const edgeRows = edges
        ...
        .sort((a, b) => (a.sourceSheet + a.sourcePath).localeCompare(b.sourceSheet + b.sourcePath, "ja"));
    return { nodeRows, edgeRows };
}
export function exportRowsToCsv(nodeRows, edgeRows) {
    const lines = ["type,id,sheet,path,related_sheet,related_path,name,description,cross_sheet"];
    for (const r of nodeRows) {
        lines.push(["node", r.id, r.sheet, r.path, "", "", r.name, r.description, ""].map(csvEscape).join(","));
    }
...
export function exportRowsToYaml(nodeRows, edgeRows) {
    const lines = ["nodes:"];
...
export function exportRowsToToml(nodeRows, edgeRows) {
    const parts = [];
    for (const r of nodeRows) {
        parts.push([
            "[[nodes]]",
...
export function exportRowsToJson(nodeRows, edgeRows) {
    return JSON.stringify({ exportedAt: new Date().toISOString(), nodes: nodeRows, edges: edgeRows }, null, 2);
}
export function serializeExportRows(format, nodeRows, edgeRows) {
    switch (format) {
        case "yaml":
            return {
...
export function exportGraphJson(nodes, edges, sheets) {
    return JSON.stringify({ version: 2, exportedAt: new Date().toISOString(), sheets, nodes, edges }, null, 2);
}
